...
import { code, stringify, assertSameCode, assertSameCodeOneOf, } from "./helpers.js";
...
export function assertCodeMatchingInPairs(pairs) {
    pairs.forEach(([sourceCode, expectedCode]) => {
        const actualCode = esmToCjs(sourceCode);
        if (typeof expectedCode === "string") {
            const msg = `${stringify(sourceCode)} does not convert to ${stringify(expectedCode)}`;
...
describe("import statements", () => {
    describe("ImportDeclaration", () => {
        ...
        it("ImportDefaultSpecifier", () => {
            const sourceCode1 = `import foo from 'mod';`;
            assertSameCode(esmToCjs(sourceCode1), `const {default: foo} = require('mod')`);
        });
        ...
        it("mixed", () => {
            ...
            assertSameCode(esmToCjs(sourceCode), `const { default: foo, bar } = require("mod")`);
        });
    });
});
...
//# sourceMappingURL=esm_to_cjs.test.js.map